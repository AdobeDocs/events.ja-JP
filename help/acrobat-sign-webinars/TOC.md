---
user-guide-title: Acrobat Sign webinars
user-guide-description: Adobeのエキスパートから、Acrobat Signで新しい電子サインワークフローを習得する方法を学びます。
cloud: Document Cloud
solution: Document Cloud
source-git-commit: 80ce8dae288f75550ac9c967cf67f542bf5e6fce
workflow-type: tm+mt
source-wordcount: '130'
ht-degree: 5%

---


# Acrobat Sign webinars {#acrobat-sign-webinars}

+ [概要](overview.md)
+ リリースウェビナー {#releases}
   + [2026年2月](skill-builders/acrobat-sign-february-2026-release.md)
   + [2025年10月](skill-builders/acrobat-sign-october-2025-release.md)
   + [2025年7月](skill-builders/acrobat-july-release.md)
+ イノベーション – 新機能のハイライト {#newfeatures}
   + [ヘルスケア、ライフサイエンス、FDA規制に対応した構成](./skill-builders/regulated-best-practices.md)
   + [カスタムワークフローの基本を学ぶ](./skill-builders/custom-workflows.md)
   + [IDの検証とセキュリティ](./skill-builders/identity-verification-security.md)
   + [モバイル電子サインの最適化](./skill-builders/mobile-e-signing.md)
   + [Web フォーム](./skill-builders/web-forms.md)
+ 101 - ユーザー向けの基本を学ぶ {#getstartedusers}
   + [Acrobat Sign101](./skill-builders/getting-started.md)
   + [オーサリング環境](./skill-builders/authoring-environment.md)
   + [契約書の電子サインの準備](./skill-builders/e-signature-setup.md)
   + [契約書ステータスの表示](./skill-builders/agreement-status.md)
   + [Microsoft アプリケーションでの電子サインの操作](./skill-builders/e-signature-microsoft.md)
   + [ワークフローの作成と使用](./skill-builders/create-use-workflows.md)
+ 101 – 管理者向け入門 {#getstartedadmins}
   + [Acrobat Sign管理エクスペリエンスをパワーアップ](./skill-builders/acrobat-sign-admin-experience.md)
   + [ETLAの管理者](./skill-builders/acrobat-sign-admin-etla.md)
   + [高度なレポート](./skill-builders/advanced-reporting.md)
+ 102 – 高度なタスク {#advancedtasks}
   + [署名用に文書を送信するための詳細なヒント](./skill-builders/advanced-sending-documents-signature.md)
   + [ワンクリックで多くの署名を収集](./skill-builders/collect-signatures.md)
   + [Document Builder](./skill-builders/document-builder.md)
   + [入力可能な署名可能なWeb フォームの投稿](./skill-builders/fillable-signable-web-form.md)
   + [ワークフローの自動化](./skill-builders/workflow-automations.md)
