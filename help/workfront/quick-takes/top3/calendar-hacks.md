---
title: Workfrontのカレンダーに関する上位 3 つの秘訣
description: Workfrontの上位のカレンダーの秘訣を見つけて、整理と効率を高めます。
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 308
last-substantial-update: 2025-12-10T00:00:00Z
jira: KT-19742
source-git-commit: df552b4f2b2fa1d8e4e8941aa9d58acecc60dd2b
workflow-type: tm+mt
source-wordcount: '93'
ht-degree: 1%

---


# 上位 3 つの Fave カレンダーの秘訣

これもまた、私が個人的に気に入っているWorkfront カレンダーのハックの例を含む「オンデマンド Workfront CSM」です。

* PTO カレンダー
* ソーシャルメディアカレンダー
* カレンダーに直接表示するリクエスト（新規従業員のオンボーディングの例）
* そして、ボーナスヒント！

>[!VIDEO](https://video.tv.adobe.com/v/3477064/?captions=jpn&learn=on&enablevpops)

その他のリソースについては、[Experience League Community の投稿をご覧くだ &#x200B;](https://experienceleaguecommunities.adobe.com/t5/workfront-discussions/video-top-3-fave-calendar-hacks/td-p/591047?profile.language=ja) い。

新しいアイデアやクリエイティブなアプローチについて学びたい場合は、[Experience League イベントページ &#x200B;](https://experienceleague.adobe.com/ja/events) で今後のワークショップに登録してください。