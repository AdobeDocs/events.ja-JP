---
title: CreativeのWorkfront ボードの使用方法
description: Mattel チームが紹介したように、Workfront Boards を使用して効果的なプロジェクト管理と共同作業を行う創造的なアプローチを見つけましょう。
role: Admin, Developer, Leader, User
level: Beginner, Intermediate
doc-type: Event
duration: 197
last-substantial-update: 2025-12-17T00:00:00Z
jira: KT-18646
source-git-commit: 9f3e518295eaf64776880ba0442eb8a1b8d645e4
workflow-type: tm+mt
source-wordcount: '133'
ht-degree: 0%

---


# ボードを使用したCreativeのアプローチのトップ 3

もう一度、オンデマンドWorkfrontCSM の Cynthia Boon が、Workfrontボードを使用するための創造的なアイデアを刺激したいと思います。  今、私はビデオでそれを言及していますが、念のために、ここではアジャイルチームで作業する必要がないことを簡単に示します。ボード機能を使用します。  プロジェクトタスクリストがチームに対して機能しない場合は、ボードに試してみます。  

>[!VIDEO](https://video.tv.adobe.com/v/3470068/?learn=on&enablevpops)

顧客がボードをどのように使用しているかの例を探している場合は、 2024Adobe Summitの Mattel チームの Boards プレゼンテーションを見てください。  

[2024 Summit Session: How Mattel Is Reveloping Workfront Boards](https://business.adobe.com/jp/summit/2024/sessions/skill-exchange-how-mattel-is-leveraging-workfront-s956.html)


