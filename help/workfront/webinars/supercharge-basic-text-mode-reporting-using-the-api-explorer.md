...
API エクスプローラー、API エクスプローラーの使用方法、基本テキストモードを活用してレポートを拡張する方法について説明します。 このウェビナーは 2020 年 1 月 22 日に録画されました。
...
複数選択可能で、最初の選択項目のみをレポートに取り込むチェックボックスフィールドについてレポートできますか？

**回答**

はい。チェックボックスフィールドで選択した選択肢は、すべて 1 つの文字列で、各選択項目がコンマで区切られます。 SEARCH 式を使用してチェックボックスフィールド内の最初のコンマの位置を検索し、そのインデックスを LEFT 式と共に使用して、リストの先頭からその多くの文字を表示します。 コードは次のとおりです。
