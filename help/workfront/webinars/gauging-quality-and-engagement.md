...
反復では、主なジョブの役割を特定できます。 これは値式で行う必要がありますが、HTMLコードは値式では認識されません。 そこで、その役割を主なものとして識別する別の方法を考え出す必要があります。 このコードの主な役割名の前後に「**」を付けます。 体験して、好みを確認してください。
...
レポートエディターで、新しい列を 2 つ追加し、列（表示）画面の左端にドラッグします。 これを行った後、左側の列にあったものを見てみると、テキストモードの列番号がすべて増加しているのがわかります。 必要な数だけ空白の列をドラッグします。 何かを保存する前に、空白の列に入れてください。入れないと、削除されます。
...
最初のレポートはプロジェクト名またはプロジェクト名に基づいていました。タスクに対しても実行できます。また、タスクをグループ化する最適な方法は、タスク名が異なる頻度が高い場合もあります。ありがとう！
