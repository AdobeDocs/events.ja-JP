---
title: ピッチパーフェクト - Workfrontの価値の定義と伝達
description: スライドを重ねてカスタマイズ可能な資料を作成しました。この資料は、お客様の組織でWorkfrontが「理由」を特定し、結果を報告し、エグゼクティブ、同業者、エンドユーザーの共感を得られる方法で成果を伝えるのに役立ちます。
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3490
last-substantial-update: 2025-12-03T00:00:00Z
jira: KT-19782
source-git-commit: 2a5336b19f61bb0e1c48311b36c76c4a0967c410
workflow-type: tm+mt
source-wordcount: '180'
ht-degree: 0%

---


# ピッチパーフェクト：Workfrontの価値を定義し、伝える

「理由」の定義から勝利のピッチの提供まで、 Workfront インスタンスは毎日価値を提供しますが、そのストーリーを、ふさわしい評価、サポート、リソースを得られる形で共有していますか？

ワークショップでは、スライドを組み合わせてカスタマイズ可能な資料を作成しました。この資料は、お客様の組織でWorkfrontが「理由」を特定し、結果を報告し、エグゼクティブ、同業者、エンドユーザーの共感を得られる方法で成果を伝えるのに役立ちます。

>[!VIDEO](https://video.tv.adobe.com/v/3477131/?learn=on&enablevpops)

[ プレゼンテーション スライド デッキ ](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/093025+-+Pitch+Perfect+-+Defining+%26+Communicating+Workfront+Value.pdf) のレビューを行い、オンデマンド録画の流れを見てみましょう。

セッション中に共有されるメモとベストプラクティスを含む、カスタマイズ可能なスライドデッキをお探しですか？  その他のリソースについては、[Experience League コミュニティの投稿 ](https://experienceleaguecommunities.adobe.com/t5/workfront-discussions/event-follow-up-pitch-perfect-defining-amp-communicating/td-p/779706) をご覧ください。

毎月、新しいイベントが追加されるので、最新のセッションについては、[Experience League イベントページ ](https://experienceleague.adobe.com/ja/events) を確認してください。


