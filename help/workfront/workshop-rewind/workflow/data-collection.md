---
title: Workfront Proのヒント - Workfrontの次世代のデータ収集の機会
description: Workfrontでのデータ収集を最適化し、効率とガバナンスを強化するための高度なテクニックをご紹介します。
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3546
last-substantial-update: 2026-03-26T00:00:00Z
jira: KT-20603
source-git-commit: c350251d880f9e1ea4de6de51e1139894c9a3b47
workflow-type: tm+mt
source-wordcount: '175'
ht-degree: 2%

---


# Workfront Proのヒント - Workfrontの次世代のデータ収集の機会

AdobeのエンタープライズアーキテクトコンテンツSupply chainであるSteve Teitelbaum氏と連携してホストされるAdobe Workfrontのカスタムフォームとビジネスルールの高度な機能を利用して、どのようにデータ収集をより詳細に制御し、運用、レポート、Workfront統合、AI ユースケースを合理化できるのかを解説します。

>[!VIDEO](https://video.tv.adobe.com/v/3481821/?learn=on&enablevpops)

このセッションでは、以前のワークショップ「[Workfront Proのガバナンス、実行、および導入のヒント ](https://experienceleague.adobe.com/en/docs/events/workfront-recordings/workshop-rewind/workflow/governance-execution-adoption)」のディスカッションについて詳しく説明します。

セッションを逃した場合、心配はありません！ [ スライドデッキ ](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/Workfront+Pro+Tips+Part+2_Dec2025.pdf)を確認し、オンデマンド録画を視聴します。

毎月新しいイベントが追加されるので、最新のセッションについては、[Experience League イベント ページ ](https://experienceleague.adobe.com/ja/events)を必ず確認してください。

