---
title: 重要な情報の測定 – Schneider ElectricがFusionでWorkfrontの効率化をどのように推進しているか
description: Schneider ElectricがWorkfront Fusionを活用して、クリエイティブスタジオのプロセスを合理化し、時間を節約して、効率性を向上させた方法をご覧ください。
feature: Workfront Fusion
topic: Performance
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 2920
last-substantial-update: 2026-03-26T00:00:00Z
jira: KT-20602
exl-id: baebbf5a-be97-4dc3-956b-4669fb8d11ee
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '157'
ht-degree: 2%

---

# 重要な情報の測定 – Schneider ElectricがFusionでWorkfrontの効率化をどのように推進しているか

Schneider ElectricのCreativeオペレーションのエキスパートであるKimberly Rea氏とAdobe Workfront Championが、Schneider ElectricがカスタムのFormsとFusionのシナリオを活用してワークフローを自動化し、自動化されたタッチポイントの量を追跡して、効果の定量化とオートメーションの拡張の事例の構築を支援する方法を解説します。

>[!VIDEO](https://video.tv.adobe.com/v/3481820/?learn=on&enablevpops)

セッションを逃した場合、心配はありません！ [ スライドデッキ ](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/120325+-+Measuring+What+Matters+-+How+Schneider+Electric+Drives+Workfront+Efficiency+with+Fusion.pdf)を確認し、オンデマンド録画を視聴します。

毎月新しいイベントが追加されるので、最新のセッションについては、[Experience League イベント ページ ](https://experienceleague.adobe.com/ja/events)を必ず確認してください。
