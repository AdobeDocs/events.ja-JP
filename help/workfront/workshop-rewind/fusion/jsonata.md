---
title: Workfront Fusion + JSONataでのJSONの実行
description: このセッションでは、JSON パス表記法の読み取り、データタイプの理解、Fusionでの適切な開始パスの選択の基本について説明しました。
feature: Workfront Fusion, System Setup and Administration
topic: Performance
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3618
last-substantial-update: 2025-12-03T00:00:00Z
jira: KT-19780
exl-id: 28081dcd-a5d1-4444-850a-237e2378e308
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '216'
ht-degree: 1%

---

# Workfront Fusion + JSONataでのJSONの実行

このセッションは、Adobe Professional ServicesのSteve Watts氏とBrian Haight氏と共同で開催され、JSON パスの表記法の読み取り、データタイプの理解、Fusionでの適切なスタートパスの選択など、基本的な知識を学びました。

さらに、データの検索とフィルタリング、JSONの再シェイプ、高度な変換の実行など、3つの実例が紹介されました。 時間を節約し、データ処理を簡素化したいFusion管理者には必須のスキルです。

>[!VIDEO](https://video.tv.adobe.com/v/3477129/?learn=on&enablevpops)

ライブに参加できなかった場合でも、心配はいりません。[&#x200B; スライドデッキ &#x200B;](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/Slide+Deck+-+JSONata+Fusion+Event+-+September+17%2C+2025.pdf)を確認し、オンデマンド録画をご覧ください。

ダウンロード可能な資料については、[Experience League コミュニティ投稿](https://experienceleaguecommunities.adobe.com/t5/workfront-discussions/event-follow-up-making-json-sing-with-workfront-fusion-jsonata/td-p/776029?profile.language=ja)を参照してください。トレーニング資料のzip ファイルに従ってください。

毎月新しいイベントが追加されるので、最新のセッションについては、[Experience League イベント ページ &#x200B;](https://experienceleague.adobe.com/ja/events)を必ず確認してください。
