---
title: Commerce Intelligenceの導入方法
description: このウェビナーでは、Adobe CommerceまたはMagento Open Source ストア向けのCommerce Intelligenceの主な機能について説明します。
landing-page-description: この録画されたウェビナーでは、Adobe CommerceまたはMagento Open Source ストアのCommerce Intelligenceの主な機能について説明します。
short-description: この録画されたウェビナーでは、Adobe CommerceまたはMagento Open Source ストアのCommerce Intelligenceの主な機能について説明します。
kt: 10408
solution: Commerce, Commerce Intelligence
feature-set: Commerce Intelligence
feature: Data Integration, Reports
topic: Commerce, Performance
role: Leader, User, Data Architect
level: Beginner
thumbnail: 342501.jpeg
exl-id: ac932e1b-ae0a-40a3-b7dd-948d88ded358
duration: 3221
source-git-commit: 76653a37bdcd3176067a4f0c3f4a073e902a63de
workflow-type: tm+mt
source-wordcount: '173'
ht-degree: 1%

---

# Commerce Intelligenceの導入方法

>[!NOTE]
>
>Adobe Commerce Intelligenceは以前[!DNL Magento Business Intelligence (MBI)]と呼ばれていました。 過去の出来事の記録は以前の名前を反映しています。

Commerce Intelligenceの主な機能について、Commerceのプロダクトチームが直接説明します。事前に設定されたダッシュボードとカスタマイズオプションについて詳しく説明します。

ユースケースは全体を通じて共有され、初日からBIを利用して、加盟店がどのようにインサイトを獲得し、より多くの成長を促進しているのかを示す実例が提供されます。

>[!VIDEO](https://video.tv.adobe.com/v/3425736?quality=12&learn=on)

## その他のリソース

[Commerce Intelligenceの概要](https://experienceleague.adobe.com/docs/commerce-business-intelligence/mbi/getting-started.html?lang=ja)
[Commerce Intelligence サポート &#x200B;](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/mbi-service-policies.html?lang=ja)
