---
title: 成功に導く顧客データプラットフォームの選定
description: 市場にはCDPに関するノイズが溢れていますが、どこに注力すべきか、成功に向けてどのように準備すればよいのかを判断するのは困難です。
solution: Real-Time Customer Data Platform
feature: Data Governance
topic: Personalization
role: User, Developer, Data Engineer, Architect, Data Architect, Admin, Leader
level: Beginner, Intermediate, Experienced
kt: 10633
thumbnail: 344312.jpeg
exl-id: 434fd1c4-e7f1-40a8-bf61-4b5a72ce3091
duration: 3479
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '150'
ht-degree: 1%

---

# 成功に導く顧客データプラットフォームの選定

市場にはCDPに関するノイズが溢れていますが、どこに注力すべきか、成功に向けてどのように準備すればよいのかを判断するのは困難です。 このセッションでは、AdobeのMatt Skinner氏とKevin Cobourn氏が、CDPが提供できる価値を紹介し、5つの重要な評価基準を提示し、CDPを導入するための準備を進めるためのステップを共有します。

>[!VIDEO](https://video.tv.adobe.com/v/344312/?quality=12&learn=on)

## 追加情報

*[顧客データ管理の人物とプロセスのベストプラクティス](people-and-process.md)
*[&#x200B; ファーストパーティデータを活用して優れたエクスペリエンスを構築](https://experienceleague.adobe.com/docs/events/customer-data-management-voices-recordings/industry/build-superb-experiences-with-your-first-party-data.html)
*[&#x200B; ユーザー事例 – TSB銀行、あらゆる顧客にリアルタイムでデータをパーソナライズ。](https://business.adobe.com/customer-success-stories/tsb-case-study.html)
