---
title: あらゆる要素をまとめる
description: Adobe Analyticsがweb サイトのデータを追跡する方法、保存、共有、共同作業の方法。
solution: Analytics
feature-set: Analytics
feature: Analytics Basics, Data Governance, Data Sources, Data Configuration and Collection
topic: Migration, Integrations
doc-type: Event
role: User
level: Beginner
kt: 10457
thumbnail: 343320.jpeg
exl-id: e6e2ebce-0fab-4c06-98a8-ec90753e551a
duration: 2141
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '53'
ht-degree: 0%

---

# あらゆる要素をまとめる

最後に、Adobe Analyticsがweb サイトのデータをどのように追跡し、どのように保存、共有、共同作業をおこなうのかを解説します。 このセッションでは、生産性を高め、学習ジャーニーを継続するためのヒントを解説します。

>[!VIDEO](https://video.tv.adobe.com/v/343320/?quality=12&learn=on)
