---
title: マーケティングチャネルとアトリビューションの習得
description: Adobe Analyticsでのマーケティングチャネル設定の複雑さを説明します。 アトリビューションモデルを活用して、マーケティング戦略に実用的なインサイトを提供します。
solution: Analytics
feature: Data Configuration and Collection
topic: Development
role: User
level: Intermediate, Experienced
doc-type: Event
duration: 2176
last-substantial-update: 2024-08-30T00:00:00Z
jira: KT-15975
exl-id: e5f614e4-6a80-40ef-a662-f1442be68f08
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '215'
ht-degree: 0%

---

# Adobe Adobe Analyticsのマーケティングチャネルとアトリビューションの習得

今日の変化の激しいデジタルマーケティングでは、利用者がweb サイトやモバイルアプリを検索してエンゲージする方法を把握することは、施策の最適化とROIの向上にとって最も重要なことです。 このセッションでは、Adobe Analyticsでマーケティングチャネルを設定し、アトリビューションモデルを活用して、マーケティング戦略に実用的なインサイトを提供するための複雑な作業について解説します。

## 重要な留意点

* マーケティングチャネル入門：デジタルプレゼンスに対する様々なマーケティングチャネルの影響を把握するために、それらのチャネルを特定し、分類する重要性を説明します。
* Adobe Analyticsの設定：キャンペーントラッキングコードの設定、関連データの取得、Adobe Launchを使用した効率的なデータ収集の実践的な知識を習得します。

  このセッションで紹介されているキャンペーントラッキングコードジェネレーターツールをダウンロードするには、[Experience Leagueの視点でAdobe Analyticsとマーケティングのチャネルとアトリビューションを習得](https://experienceleague.adobe.com/en/perspectives/mastering-adobe-analytics-marketing-channels-attribution)するをご覧ください。

* 分類と処理ルール：分類ルールビルダーの使用方法と、正確なトラフィックソース分類のためのマーケティングチャネル処理ルールの設定方法について説明します。
* アトリビューションモデル：Adobe Analyticsで利用できるさまざまなアトリビューションモデルを検討し、それらを活用してマーケティング活動の効果を測定し、最適化する方法を探ります。

>[!VIDEO](https://video.tv.adobe.com/v/3432747/?learn=on)
